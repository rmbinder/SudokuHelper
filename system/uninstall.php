...
/**
 ***********************************************************************************************
 * Uninstallation of the Admidio plugin SudokuHelper
 *
...
try {
    ...
    $result = $gL10n->get('PLG_SUDOKU_HELPER_UNINST_STARTMESSAGE');
    ...
    switch ($getMode) {
        case 'security_check':
            ...
            $title = $gL10n->get('PLG_SUDOKU_HELPER_UNINSTALLATION');
            $headline = $gL10n->get('PLG_SUDOKU_HELPER_UNINSTALLATION');

            $gNavigation->addUrl(CURRENT_URL, $headline);

            // create html page object
            $page = PagePresenter::withHtmlIDAndHeadline('plg-sudokuhelper-uninstall-html');
            $page->setTitle($title);
            $page->setHeadline($headline);

            $form = new FormPresenter('sudokuhelper_uninstall__form', '../templates/uninstall.plugin.sudokuhelper.tpl', '', $page, array(
                'type' => 'default',
                'method' => 'post',
                'setFocus' => false
            ));
        ...
        case 'uninst':
            ...
            if ($pPreferences->config['install']['access_role_id'] == 0 || $pPreferences->config['install']['menu_item_id'] == 0) {
                ...
                $result .= $gL10n->get('PLG_SUDOKU_HELPER_UNINST_NO_INST_IDS_FOUND');
            } else {
                $result_role = true;
                ...
                $result .= ($result_role ? $gL10n->get('PLG_SUDOKU_HELPER_UNINST_ACCESS_ROLE_SUCCESS') : $gL10n->get('PLG_SUDOKU_HELPER_UNINST_ACCESS_ROLE_ERROR'));
                ...
                if (count($access_roles_prefs) === 1 && $access_roles_prefs[0] == $pPreferences->config['install']['access_role_id']) {
                    ...
                    $result .= ($result_menu ? $gL10n->get('PLG_SUDOKU_HELPER_UNINST_MENU_ITEM_SUCCESS') : $gL10n->get('PLG_SUDOKU_HELPER_UNINST_MENU_ITEM_ERROR'));
                } else {
                    // Menüpunkt nicht entfernen, wenn er noch für eine andere Organisation verwendet wird
                    $result .= $gL10n->get('PLG_SUDOKU_HELPER_UNINST_MENU_ITEM_NOT_DELETED');
                }
            }
            ...
            $result .= ($result_data ? $gL10n->get('PLG_SUDOKU_HELPER_UNINST_DATA_DELETE_SUCCESS') : $gL10n->get('PLG_SUDOKU_HELPER_UNINST_DATA_DELETE_ERROR'));
            $result .= ($result_db ? $gL10n->get('PLG_SUDOKU_HELPER_UNINST_TABLE_DELETE_SUCCESS') : $gL10n->get('PLG_SUDOKU_HELPER_UNINST_TABLE_DELETE_ERROR'));
