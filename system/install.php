...
/**
 ***********************************************************************************************
 * Installation routine for the Admidio plugin SudokuHelper
 *
 * @copyright rmb
...
try {
    ...
    $role->readDataByColumns(array(
        ...
        'rol_name' => $gL10n->get('PLG_SUDOKU_HELPER_ACCESS_TO_PLUGIN'),
        'rol_description' => $gL10n->get('PLG_SUDOKU_HELPER_ACCESS_TO_PLUGIN_DESC')
    ));
    ...
    if ($role->getValue('rol_id') === 0) // nur wenn es keine Rolle gibt, neue Daten eingeben (mehr als eine Rolle wird nicht betrachtet)
    {
        ...
        $role->setValue('rol_name', $gL10n->get('PLG_SUDOKU_HELPER_ACCESS_TO_PLUGIN'));
        $role->setValue('rol_description', $gL10n->get('PLG_SUDOKU_HELPER_ACCESS_TO_PLUGIN_DESC'));
        $role->save();
    }
    ...
    $menu->setValue('men_icon', 'puzzle');
    $menu->setValue('men_name', 'PLG_SUDOKU_HELPER_NAME');
    $menu->setValue('men_description', 'PLG_SUDOKU_HELPER_NAME_DESC');
    $menu->save();
